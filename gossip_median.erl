...
compute_local_median(MyMedianComputation, NodesMedianComputation) ->
		...
		true -> 
			...
			true ->
				...
				NewFragment = lists:sort(lists:merge(lists:map(fun(_) -> Median end, lists:seq(1,MedianRepetitions)),
									lists:map(fun(_) -> SumPreservationValue end, lists:seq(1, NoOfValues - MedianRepetitions))
									)
							)
			end,
	...
	true ->
		...
		true ->
			...
			NewFragment = lists:sort(lists:merge(lists:map(fun(_) -> Median end, lists:seq(1,MedianRepetitions)),
									lists:map(fun(_) -> SumPreservationValue end, lists:seq(1, (MyLength - MedianRepetitions))
								)
						))
			end,
...
init_the_dhondus(N) ->
	...
	Fragments = lists:map(fun(_) -> lists:map(fun(_) -> random:uniform(10)+0.5 end, lists:seq(1, 3)) end, lists:seq(0,N-1)),
	
	
	%FragmentsIdMap = lists:map(fun(_)-> random:uniform(erlang:trunc(N/2)) end, lists:seq(1,N)),
	Pids = lists:map(fun(Node) -> spawn(gossip_median, gossip, [lists:map(fun(_) -> random:uniform(N) end, lists:seq(0,erlang:trunc(math:log(N)))), 
								N, 
