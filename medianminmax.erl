...
gossip(MyPartition, N, MyFragment, MyMedianComputation, Mode, RoundsRemaining, Delay) ->
	%if(length(FragmentsImFetching)>0)->
		...
		Mode == readyToSend, RoundsRemaining == 0 ->
			...
			io:format("Local Median Computation at ~p : ~p~n", [self(), lists:nth(2, MyMedianComputation)]),
	...
	receive			
		...
		after 2000->
			...
			true->
				gossip(MyPartition, N, MyFragment, MyMedianComputation, readyToSend, RoundsRemaining-1, 0)
...
compute_local_median(MyMedianComputation, NodesMedianComputation) ->
	if length(MyMedianComputation)==1 ->
		...
		Min = lists:min(SortedFragment),
		Max = lists:max(SortedFragment),
		...
		true -> 
			...
			true ->
				...
				if NoOfValues rem 2 ==0 ->
					Primary = [Min, Max, Median, Median],

					if NoOfValues == 4 ->
						NewFragment = Primary;
					true ->
						BalanceCount = erlang:trunc((NoOfValues-4)/2),
						%LeftBalance = (LeftSum - Min)/ BalanceCount,
						%RightBalance = (LeftSum - Max)/ BalanceCount,

						LeftBalance = (Min+Median)/2,
						RightBalance = (Max+Median)/2,
						SecondaryLeft = lists:map(fun(_) -> LeftBalance end, lists:seq(1,BalanceCount)),
						SecondaryRight = lists:map(fun(_) -> RightBalance end, lists:seq(1,BalanceCount)),
						Secondary = lists:merge(SecondaryLeft, SecondaryRight),
						NewFragment = lists:sort(lists:merge(Primary, Secondary))
					end;

				true ->
					Primary = [Min, Max, Median],

					if NoOfValues == 3 ->
						NewFragment = Primary;
					true ->
						BalanceCount = erlang:trunc((NoOfValues-3)/2),
						%LeftBalance = (LeftSum - Min)/ BalanceCount,
						%RightBalance = (LeftSum - Max)/ BalanceCount,
			...
			%[NewFragment, Median, SumOfValues, NoOfValues]
			[NewFragment, Median, SumOfValues, NoOfValues]
		end;
	...
	true ->
		...
		%[NewFragment, Median, SumOfValues, MyLength]
		[NewFragment, Median, SumOfValues, NoOfValues]
	end
.
...
init_the_dhondus(N) ->
	...
	Pids = lists:map(fun(Node) -> spawn(medianminmax, gossip, [lists:map(fun(_) -> random:uniform(N) end, lists:seq(0,erlang:trunc(math:log(N)+1))), 
								N, 
